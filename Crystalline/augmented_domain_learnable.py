"""Augmented NODEs with learnable system embeddings.

This module is similar to :mod:`augmented_domain` but treats the
system embeddings as trainable parameters.  A small projection network
can lift the one‑hot vectors to a higher‑dimensional latent space,
allowing the model to learn relationships between different systems.
"""
...
class SystemAugmentedNeuralODE(eqx.Module):
    ...
    def __call__(self, ts: Array, y0: Array, *, system_embedding: Array, return_augmented: Bool = False) -> Array:
        """
        Args:
            ts: Time points of shape (T,)
            y0: Initial condition in ℝ^d (original space), shape (d,)
            system_embedding: System embedding vector, shape (system_embed_size,)
        ...
        """
        # Augment initial condition: [y0; zeros_aug; system_embedding]
...
# ────────────────────────────────────────────────────────────────────────────────
# Learnable wrapper for system embeddings
# ────────────────────────────────────────────────────────────────────────────────
class LearnableSystemAugmentedModel(eqx.Module):
    """
    Composite model that couples a System‑Augmented Neural ODE (SA‑NODE) with a
    *learnable* table of system embeddings.

    ▸ Back‑compatibility:
      • If the caller passes an explicit ``system_embedding=…`` keyword (e.g.
        the existing plotting utilities) we simply forward that vector to the
        ODE and **do not** touch the internal table.
      • During training you can provide a scalar ``system_id`` instead.  The
        corresponding row of ``self.embeddings`` is looked‑up *dynamically*
        so gradients propagate and Optax can update the table.

    Args
    ----
    ode:
        An instance of :class:`SystemAugmentedNeuralODE`.
    embeddings:
        2‑D array of shape ``(num_systems, embed_dim)``.  It is initialised to
        one‑hot rows but **is NOT** a static field so that it remains trainable.
    embed_proj:
        Projects raw embedding → high‑dimensional.

    `embed_proj` lifts the one‑hot system vector to a trainable, higher‑dimensional latent (default 16) that is passed to the ODE.
    """

    ode: SystemAugmentedNeuralODE
    embeddings: Array  # shape = (N_sys, d_embed)
    embed_proj: eqx.nn.Linear  # projects raw embedding → high‑dimensional
    ...
    def __call__(
        ...
        y0: Array,
        *,
        system_id: Array | int | None = None,
        system_embedding: Array | None = None,
    ) -> Array:
        # ─────────────────── Compatibility branch ───────────────────
        if system_embedding is not None:
            # Embedding supplied externally – just forward it.
            return self.ode(ts, y0, system_embedding=system_embedding)

        # ─────────────────── Learning branch ────────────────────────
        if system_id is None:
            raise ValueError("Either `system_id` or `system_embedding` must be provided.")

        # Convert to array so that `vmap` + `jit` work properly
        system_id = jnp.asarray(system_id, dtype=jnp.int32)

        # Dynamic row selection – gradients flow into the looked‑up row
        embed_vec = self.embeddings[system_id]
        # Linear projection to richer space
        embed_vec = self.embed_proj(embed_vec)

        return self.ode(ts, y0, system_embedding=embed_vec)
...
def train_SystemAugNODElearnable_fresh(
    ...
    *,
    ...
    output_constraints: dict | None = None,
    system_embeddings: dict[int, Array] | None = None,
) -> tuple[Array, Array, Array, SystemAugmentedNeuralODE, dict[int, MinMaxScaler | StandardScaler] | None, list[dict]]:
    """
    Trains a System-Augmented Neural ODE model from scratch on multi-system crystallization data.

    Args:
        embedding_proj_size: int, optional. Output dimension of the linear projection that lifts one-hot system embeddings to a higher-dimensional latent passed to the ODE (default 16).
    """
    key = jr.PRNGKey(seed)
    data_key, model_core_key, proj_key, loader_key = jr.split(key, 4)
    ...
    if verbose:
        print("Using system embeddings:", system_embeddings.keys())
        print("System embedding size:", len(next(iter(system_embeddings.values()))))

    raw_embed_size = len(next(iter(system_embeddings.values())))  # one‑hot dim
    system_embed_size = embedding_proj_size  # rich dim passed to ODE
    num_systems = len(system_embeddings)
    # One‑hot initialisation – will be *learnable* through the new wrapper
    initial_embeddings = jnp.stack([system_embeddings[i] for i in range(num_systems)])
    ...
    if model is None:
        # 1) Core ODE expects `system_embed_size` = rich dimension
        ode_core = SystemAugmentedNeuralODE(
            data_size,
            augment_dim,
            system_embed_size,
            width_size,
        ...
        # 2) Linear projector from one‑hot → rich space
        embed_proj = eqx.nn.Linear(
            in_features=raw_embed_size,
            out_features=system_embed_size,
            key=proj_key,
        )
        model = LearnableSystemAugmentedModel(
            ode=ode_core,
            embeddings=initial_embeddings,
            embed_proj=embed_proj,
        )
    else:
        augment_dim = model.ode.augment_size
        system_embed_size = model.ode.system_embed_size

    @eqx.filter_value_and_grad
    def grad_loss(model, ti, yi, system_ids_batch):
        def predict_single(y0, sid):
            return model(ti, y0, system_id=sid)

        # Vectorise across the batch dimension
        y_pred = jax.vmap(predict_single)(yi[:, 0], system_ids_batch)

        # Mean‑squared error over all time steps & state dimensions
    ...
    try:
        ...
        with training_progress:
            for lr, steps, length in zip(lr_strategy, steps_strategy, length_strategy, strict=True):
                ...
                optim = optax.chain(
                    optax.clip_by_global_norm(1e-2),
                    optax.adabelief(
                        lr,
                    ),
                )
...
if __name__ == "__main__":
    ...
    # Trial 81 finished with value: 0.3453234757009224 and parameters:
    #
    # {'lr1': 0.001985050272284183, 'lr2': 0.0005087325320283898, 'epochs1': 1850, 'epochs2': 1950, 'width_size': 64, 'depth': 5, 'augment_dim': 3}. Best is trial 81 with value: 0.3453234757009224.
    results = train_SystemAugNODElearnable_fresh(
        # [training_data[0]],
        training_data,
        testing_data,
        augment_dim=8,
        ...
        output_constraints={0: "none", 1: "pos"},
    )

    # # ────────────────────────────────────────────────────────────────
    # # Inspect *learned* system embeddings and run a quick prediction
    # # ────────────────────────────────────────────────────────────────
    # ts, ys_train_out, ys_test_out, trained_model, trained_scalers, metrics_out = results

    # # Print the embedding matrix after training
    # print("\n=== Learned System Embeddings ===")
    # for sid, embed in enumerate(trained_model.embeddings):
    #     print(f"System {sid}: {embed}")

    # # Predict a trajectory using `system_id` (no manual embedding needed)
    # example_sys_id = testing_data[0][1]  # first system in test set
    # example_y0 = ys_test_out[0, 0]  # initial state of first test trajectory
    # pred_traj = trained_model(ts, example_y0, system_id=example_sys_id)

    # print(f"\nExample prediction for system {example_sys_id} (first 5 timesteps):")
    # print(pred_traj[:5])
